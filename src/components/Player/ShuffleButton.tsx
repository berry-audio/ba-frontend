 ...
 */
const ShuffleButton = () => {
  const { shuffle_mode } = useSelector((state: any) => state.player);
  const { source } = useSelector((state: any) => state.source);

  const { setRandom } = useTracklistService();

  const [shuffleMode, setShuffleMode] = useState<SHUFFLE_MODE>(SHUFFLE_MODE.SHUFFLE_OFF);
  ...
  const onClickShuffle = async () => {
    try {
      const nextMode = shuffleMode === SHUFFLE_MODE.SHUFFLE_OFF ? SHUFFLE_MODE.SHUFFLE_ON : SHUFFLE_MODE.SHUFFLE_OFF;
   ...
   */
  const RenderShuffle = () => {
    const iconClass = shuffleMode === SHUFFLE_MODE.SHUFFLE_OFF ? "" : "text-primary";
  ...
  return (
    <ButtonIcon className="w-12 h-12" onClick={onClickShuffle} disabled={!source?.controls?.includes("shuffle")}>
      <RenderShuffle />
    </ButtonIcon>
  );
};
