import { getBitDepth, getBitrate, getChannels, getCodecName, getSampleRate } from "@/util";
import { DotIcon } from "@phosphor-icons/react";
import { useSelector } from "react-redux";

const StreamInfo = ({ channels = false }: { channels?: boolean }) => {
  const { current_track } = useSelector((state: any) => state.player);

  return (
    <div className="flex justify-center items-center">
      {current_track?.track?.audio_codec && getCodecName(current_track?.track?.audio_codec)}
      ...
      {current_track?.track?.channels && channels &&(
        <>
          <DotIcon size={32} className="-mx-2" />
          {getChannels(current_track?.track?.channels)}
        </>
      )}
